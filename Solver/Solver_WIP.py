...
def MoveBox(map, boxPosition, direction):
    ...
    if direction == "down":
        ...
        raise Exception(
            "Sorry, i don't know where you want to go: {0}".format(direction)
        )

    if (
        int(nextPosition["column"]) < 0
        or int(nextPosition["row"]) < 0
        or int(nextPosition["row"]) > len(map)
        or int(nextPosition["column"]) > len(map[0])
    ):
        return False
